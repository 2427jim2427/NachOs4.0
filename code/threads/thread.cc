...
Thread::setPriority(int P) {
    // DEBUG(dbgschedule, "[C] Tick ["<<kernel->stats->totalTicks<<"] thread["<<ID<<"] changes its priority from "<<Priority<<" to "<<P );
        
    Priority = P;
}
void
Thread::setOPriority(int P) {
    // DEBUG(dbgschedule, "[C] Tick ["<<kernel->stats->totalTicks<<"] thread["<<ID<<"] changes its priority from "<<Priority<<" to "<<P );
...
double
Thread::setBurst(double B) {
     Burst_time = Burst_time*0.5 + B*0.5;
     return Burst_time;
}

void
Thread::setWait(int W) {
    // DEBUG(dbgschedule, "[C] Tick ["<<kernel->stats->totalTicks<<"] thread["<<ID<<"] changes its priority from "<<Priority<<" to "<<P );
    Wait_time = W;
}

void
Thread::setLWait(int W) {
    // DEBUG(dbgschedule, "[C] Tick ["<<kernel->stats->totalTicks<<"] thread["<<ID<<"] changes its priority from "<<Priority<<" to "<<P );
    last_wait_time = W;
}
...
Thread::Yield ()
{
    ...
    // DEBUG(dbgschedule, "finishing Yielding\n" );
}
