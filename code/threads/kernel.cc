...
Kernel::Kernel(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-rs") == 0) {
            ...
            Fpriority[execfileNum] = atoi(argv[++i]);
            cout << execfile[execfileNum] << " with priority "<< Fpriority[execfileNum] <<"\n";

		} else if (strcmp(argv[i], "-e") == 0) {
        	execfile[++execfileNum]= argv[++i];
			cout << execfile[execfileNum] << "gogo\n";
...
int Kernel::Exec(char* name)
{
    ...
    t[threadNum]->setPriority(Fpriority[threadNum]);
    t[threadNum]->setOPriority(Fpriority[threadNum]);
    // cout << name << " " << Fpriority[threadNum]<<"\n";
    //debug thread[threadNum] change priority ......
...
OpenFile* Kernel::Open(char *name)
{
