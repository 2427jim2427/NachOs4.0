...
AddrSpace::~AddrSpace()
{
    for(int i = 0; i < numPages; i++)
        AddrSpace::allocatedFrames[pageTable[i].physicalPage] = false;
    delete pageTable;
}

bool
AddrSpace::allocatedFrames[NumPhysPages] = {0};
...
// how big is address space?
    ...
    for (int i = 0; i < numPages; i++) {
        ...
        for (int j = 0; j < NumPhysPages; j++) {
            if (!allocatedFrames[j]) {   
                foundFrame = j;
                allocatedFrames[j] = true;  
                break;
            }
        }

        if (foundFrame == -1) {
            cerr << "Out of memory: not enough physical frames available\n";
            return FALSE;
        }

        pageTable[i].physicalPage = foundFrame;
        bzero(&(kernel->machine->mainMemory[foundFrame * PageSize]), PageSize);
